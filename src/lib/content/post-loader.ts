...
export async function postDetail(id: string, locale: Locale): Promise<PostModel> {
  ...
  if (!record.authorId || !authors[record.authorId] || !authorRecords[record.authorId])
    throw new Error(`Missing author for ${id}`);
  return {
    ...postCard(id, locale),
    seo: copy.seo,
    author: { ...authors[record.authorId], ...authorRecords[record.authorId] },
